...
-- 1. `1`以上`limit`未満である
-- 2. `dvs`の倍数である
make_list_Multiples :: Integer -> Integer -> [Integer]
make_list_Multiples dvs limit = [n | n <- [1..limit], mod n dvs == 0 && n < limit]

sum_list_Multiple :: Integer -> Integer -> Integer
sum_list_Multiple dvs limit = sum $ make_list_Multiples dvs limit
