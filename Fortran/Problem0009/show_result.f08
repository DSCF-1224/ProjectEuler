...
subroutine Open_FileSave (sum_target)
  ...
  write(unit=sum_target_chr, fmt='(I10.10)') sum_target
...
subroutine show_result_each (version, sum_target)
  ...
  select case (version)
    ...
    case (1)
      ...
      result = Problem0009_01 (sum_target)
      call SYSTEM_CLOCK_UsingType (System_Clock_End)

    case (2)
      
      call SYSTEM_CLOCK_UsingType (System_Clock_Start)
      result = Problem0009_02 (sum_target)
      call SYSTEM_CLOCK_UsingType (System_Clock_End)
  ...
  write (unit=SAVE_UNIT, fmt='(A9,1X,":",1X,L20)', advance='yes') 'IS FOUND', result % isfound

  if ( result % isfound ) then
    write (unit=SAVE_UNIT, fmt='(A9,1X,":",1X,I20)', advance='yes') 'a',       result % min
    write (unit=SAVE_UNIT, fmt='(A9,1X,":",1X,I20)', advance='yes') 'b',       result % mid
    write (unit=SAVE_UNIT, fmt='(A9,1X,":",1X,I20)', advance='yes') 'c',       result % max
    write (unit=SAVE_UNIT, fmt='(A9,1X,":",1X,I20)', advance='yes') 'SUM',     result % sum
    write (unit=SAVE_UNIT, fmt='(A9,1X,":",1X,I20)', advance='yes') 'PRODUCT', result % prd
  end if
